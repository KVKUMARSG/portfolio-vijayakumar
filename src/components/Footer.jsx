          ...
          <div className="flex justify-center gap-4 flex-wrap">
            {socialLinks.map((social, index) => (
              <a
                ...
                className={`relative group bg-gradient-to-r ${social.color} ${social.hoverColor} p-4 rounded-2xl transition-all duration-300 transform hover:scale-110 shadow-lg`}
                aria-label={social.label}
              >
                <social.icon size={28} className="text-white" />
                <span className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs px-3 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                  {social.label}
                </span>
              </a>
            ))}
